...
public class Simulation extends Tool
{
	...
	public static void showSimulationData(Stimuli sd, WaveformWindow ww)
	{
		...
		ww = (WaveformWindow)wf.getContent();
                ...
                // if a digital simulation, put bussed signals together and be sure to update the signal tree
 		if (!sd.isAnalog()) { makeBussedSignals(an); ww.getWindowFrame().wantToRedoSignalTree(); }
		...
		} else
		{
			...
			// makeBussedSignals(an); // was here but has been moved before checking signals in cell
		...
		for(int i=0; i<signals.size(); i++)
		{
			...
			if (numSignals <= 1) continue;
                        ...
                        for(int k=j-1; k>=i; k--)
                        {
                            ...
                            String name = subSig.getSignalName();
                            int closingBracketPos = name.lastIndexOf(']');
                            int bit = Integer.parseInt(name.substring(thisBracketPos+1, closingBracketPos));
                            if(bit!=lastbit-1 || k==i)
                            {
                                if(bit==lastbit-1)
                                    bits += ":";
                                else if(lastbit!=-1) {
                                    bits += ":" + Integer.toString(lastbit) + ",";
                                }
			...
			busSig.setSignalName(prefix+"["+bits+"]");
