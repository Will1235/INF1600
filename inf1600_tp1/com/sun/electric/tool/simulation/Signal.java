...
public class Signal
{
	...
	/** the number of busses that reference this signal */			private int busCount;
        /** a list of busses that reference this signal */			private List<Signal> parentBusses;
	...
	public void clearBussedSignalList()
	{
		for(Signal sig : bussedSignals)
			...
			sig.busCount--;
                        if(sig.busCount == 0)
                            sig.parentBusses.clear();
                        else
                            sig.parentBusses.remove(this);
                }
		bussedSignals.clear();
	}
	...
	public void addToBussedSignalList(Signal ws)
	{
		...
		ws.busCount++;
                if(ws.parentBusses == null) ws.parentBusses = new ArrayList<Signal>();
                ws.parentBusses.add(this);
	}
	...
	/**
	 * Method to return the list of busses that include this signal.
	 * @return a List of signal that are busses that include this signal.
	 */
	public List<Signal> getParentBusses() { return parentBusses; }
