...
public class SpiceOut extends Simulate
{
		...
		throws IOException
	{
		...
		List<String> signalNames = new ArrayList<String>(); // **moi: liste des signaux
		for(;;)
		{
			String line = getLine();
			if (line == null) break;
...
//				}
				...
				while (ptr < len)
				{
					...
					int start = ptr;
					while (ptr < len && !Character.isWhitespace(line.charAt(ptr))) ptr++;
					signalNames.add(new String(line.substring(start, ptr)));
					ptr++;
				}
				// **moi: ^^^
				pastEnd = true;
		...
		for(int j=0; j<mostSignals-1; j++) // **moi: le -1
		{
			AnalogSignal as = new AnalogSignal(an);
			if (j < signalNames.size()) as.setSignalName(signalNames.get(j)); // **moi: prend le nom s'il existe
			else
