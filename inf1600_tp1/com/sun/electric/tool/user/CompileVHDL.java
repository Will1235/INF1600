...
public class CompileVHDL
{
	...
	/** flag the entity as written */	private static final int     ENTITY_WRITTEN		= 0x0002;
        /** flag the entity as in list for writing */ private static final int     ENTITY_IN_LIST	= 0x0004;
...
//	/** a general flip-flop */			private static final int BLOCKFLOP     = 12;
	...
	private void doScanner(String [] strings)
	{
		...
		for(;;)
		{
			...
			if (Character.isLetter(c) || c=='\\') // in VHDL anything can be an identifier if between backslash
			{
				...
				{
					bufPos++;
					for(end=bufPos; end < buf.length() && (buf.charAt(end) != '\\' || end+1<buf.length() && buf.charAt(end+1) == '\\'); end++);
				} else {
					for(end=bufPos; end < buf.length(); end++)
					{
						char eChar = buf.charAt(end);
						if (!Character.isLetterOrDigit(eChar) && eChar != '_' && eChar != '[' && eChar != ']') break;
					}
				}
	...
	private List<String> genALS(Library destLib, Library behaveLib)
	{
			...
			System.out.println("ERROR - Cannot find interface to rename main."); else
		{
			...
			for (DBInterface interfacef = theUnits.interfaces; interfacef != null;
				interfacef = interfacef.next) interfacef.flags &= ~(ENTITY_WRITTEN|ENTITY_IN_LIST);
		...
		for (UnResList uList = unResolvedList; uList != null; uList = uList.next)
		{
			...
			if (addNetlist(destLib, gate, netlist))
			{
				...
				continue;
                        ...
                        { // then check if we can generate ALS for it
                        SymbolTree symbol = searchSymbol(gate, globalSymbols);
			if (symbol != null && symbol.pointer != null)
                        {
                            genALSInterface((DBInterface)symbol.pointer, gate, netlist); // this might add elements to unResolvedList (at the end of the list)
                            uList.numRef = 0;
                            total--;
                            continue;
                        }
...
*/			return true;
	...
	private void genALSInterface(DBInterface interfacef, String name, List<String> netlist)
	{
		...
		if (interfacef.bodies != null && interfacef.bodies.statements != null)
		{
			for (DBInstance inst = interfacef.bodies.statements.instances; inst != null; inst = inst.next)
			{
				...
				{
                                    if((((DBInterface)symbol.pointer).flags & (ENTITY_WRITTEN|ENTITY_IN_LIST)) == 0)
                                        addToUnresolved(inst.compo.name); // add it if it's not already there, for later generation
				}
			}
		}
	...
	{
                UnResList last=null; // add at end, so we can add while we scan the list
		for (UnResList uList = unResolvedList; uList != null; uList = uList.next)
		{
                        last = uList;
		...
		uList.numRef = 1;
		uList.next = null; //unResolvedList;
                if(last!=null) last.next = uList;
                else unResolvedList = uList;
	}
}
